...
def detail(request, pk):
  ...
  # comment_set --> 역참조를 위한 QuerySet API
...
def create(request):
  ...
  if request.method == 'POST':
    ...
    # 3. .....
    if form.is_valid():
      # 바로 DB에 자동으로 저장 --> request.user정보를 수동으로 저장할 필요 있음 => commit=False 이용
      article = form.save(commit=False)
      # request.user --> 로그인 한 사용자
...
def comments_create(request, pk):
  ...
  if comment_form.is_valid():
    # 댓글을 바로 DB에 저장하지 않고 수동으로 저장
    comment = comment_form.save(commit=False) 
    comment.article = article # 게시글의 외래 키
    comment.user = request.user # 로그인한 사용자
...
def likes(request, article_pk):
  ...
  # 현재 로그인 한 사용자가 좋아요를 이미 눌렀는지 확인
  if request.user in article.like_users.all():
    # 좋아요 이미 눌렀다면, 좋아요 취소(현재 로그인한 사용자를 제거)
