...
# 로그인을 하지 않은 상태에서 로그아웃 URL 접근하는 것을 방지
# 보안성: 로그인 한 사용자만 로그아웃 할 수 있도록
@login_required
def logout(request):
  auth_logout(request)
...
def signup(request):
#   is_authenticated 속성: 이미 인증된 사용자라면 -> 이미 있는 계정이면
  ...
  if request.method == 'POST': # 제출 버튼을 눌렀을 때
    ...
    if form.is_valid(): # 유효성 검사
      ...
      return redirect('articles:index')
  else: # 회원가입 폼에 들어갔을 때(제출 버튼 누르기 전)
